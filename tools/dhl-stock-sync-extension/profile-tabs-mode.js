(() => {
  ...
  function signature(profiles, selectedId, labels) {
    return JSON.stringify({
      selectedId,
      labels,
      profiles: profiles.map((p) => ({ id: text(p && p.id), name: text(p && p.name) }))
    });
  }
  ...
  async function render() {
    ...
    try {
      ...
      const sig = signature(profiles, selectedId, labels);

      if (quick.dataset.profileTabsSignature === sig && quick.querySelector('#profileQuickTabButtons')) {
      ...
      quick.dataset.profileTabsSignature = sig;
      original.style.display = 'none';
      return true;
    } finally {
      rendering = false;
  ...
  function install() {
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (changes[STORAGE_KEY] || changes[SELECTED_KEY]) scheduleRender();
    });
  }
