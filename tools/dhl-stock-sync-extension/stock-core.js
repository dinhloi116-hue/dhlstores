(function (root, factory) {
  ...
  function normalizeSku(value) { return normalizeText(value).toUpperCase(); }

  function parseVariantName(fullName, parentName) {
    const full = normalizeText(fullName);
    const parent = normalizeText(parentName);
    let tail = full;
    if (parent && full.toLowerCase().startsWith((parent + ' - ').toLowerCase())) tail = full.slice(parent.length + 3);
  ...
  function normalizeVariant(data, parentId, parentName) {
    ...
    const id = Number(data.id), available = Number(data.available);
  ...
  async function collectVariants({
    ...
    const expectedCount = Math.max(0, Number(expectedVariantCount) || 0);
    const byId = new Map(), errors = [];
    let duplicateStreak = 0, firstSeenId = null, stopReason = 'max-requests', requestCount = 0;
    ...
    for (let index = 0; index < maxRequests; index += 1) {
      ...
      if (variant.parentId && variant.parentId !== expectedParentId) {
        errors.push({ index, message: `Nguồn trả sai parentId: chờ ${expectedParentId}, nhận ${variant.parentId}`, variantId: variant.id });
        stopReason = 'parent-mismatch'; break;
      }

      if (firstSeenId == null) firstSeenId = variant.id;
      const existed = byId.has(variant.id);
      if (existed) duplicateStreak += 1;
      else { duplicateStreak = 0; byId.set(variant.id, variant); }

      if (typeof onProgress === 'function') {
        onProgress({ index: index + 1, unique: byId.size, expected: expectedCount || null, variant, existed });
      }

      // Khi file Sapo cho biết cần bao nhiêu biến thể của đội này, dùng con số đó làm chuẩn hoàn tất.
      // Nhờ vậy tool không dừng quá sớm ở vòng lặp màu đầu tiên.
      if (expectedCount > 0 && byId.size >= expectedCount) {
        stopReason = 'expected-count';
        break;
      }

      // Không có file Sapo/hint thì giữ cơ chế cũ: thấy quay lại variant đầu tiên là đủ cho test 1 sản phẩm.
      if (expectedCount === 0 && existed && variant.id === firstSeenId && byId.size > 1) {
        stopReason = 'cycle';
        break;
      }
    ...
    const reachedExpected = expectedCount > 0 && byId.size >= expectedCount;
    const completedByCycle = expectedCount === 0 && stopReason === 'cycle' && byId.size > 1;
    const confidence = (reachedExpected || completedByCycle) && errors.length === 0
      ? 'high'
      : byId.size > 0 && errors.length < 3 ? 'medium' : 'low';
    ...
    return {
      ...
      stopReason,
      confidence,
      expectedVariantCount: expectedCount,
      reachedExpected,
  ...
  function validateScanResult(result) {
    const issues = [], variants = (result && result.variants) || [], skuMap = new Map();
    const expectedParentId = Number(result && result.parentId) || null;
    const expectedCount = Math.max(0, Number(result && result.expectedVariantCount) || 0);

    if (!variants.length) issues.push({ type: 'no-variants', message: 'Không đọc được biến thể nào' });
    if (expectedCount > 0 && variants.length < expectedCount) {
      issues.push({ type: 'expected-count-missing', expected: expectedCount, actual: variants.length, message: `File Sapo cần ${expectedCount} biến thể nhưng nguồn mới đọc được ${variants.length}` });
    }

    for (const variant of variants) {
      if (expectedParentId && Number(variant.parentId) !== expectedParentId) {
        issues.push({ type: 'parent-mismatch', variantId: variant.id, message: `Variant ${variant.id} không thuộc parent ${expectedParentId}` });
      }
      if (!variant.sku) { issues.push({ type: 'missing-sku', variantId: variant.id, message: 'Biến thể thiếu SKU/code' }); continue; }
      ...
      } else skuMap.set(key, variant.id);
    ...
    if (!result || result.confidence !== 'high' || result.complete !== true) {
      issues.push({ type: 'not-complete', confidence: result && result.confidence, stopReason: result && result.stopReason, message: 'Chưa xác nhận đã đọc đủ toàn bộ biến thể của sản phẩm' });
    }
    if (result && Array.isArray(result.errors) && result.errors.length) {
      issues.push({ type: 'source-errors', count: result.errors.length, message: `Có ${result.errors.length} lỗi đọc nguồn` });
    }
    return { issues, safeToSync: issues.length === 0 };
  }

  function buildSapoSkuIndex(sapoBySku) {
    const index = {};
    if (!sapoBySku) return index;
    for (const [sku, value] of Object.entries(sapoBySku)) index[normalizeSku(sku)] = value;
    return index;
  }

  function diffInventory(sourceVariants, sapoBySku) {
    const rows = [], sapoIndex = buildSapoSkuIndex(sapoBySku);
    for (const variant of sourceVariants || []) {
      ...
      rows.push({ ...variant, sapoInventory, delta: sapoInventory == null ? null : variant.available - sapoInventory, matched: Boolean(sapo) });
    }
    return rows;
  }
  ...
  function summarize(rows) {
    ...
    return list.reduce((acc, row) => {
      acc.total += 1;
      if (row.available <= 0) acc.outOfStock += 1; else acc.inStock += 1;
  ...
  return { sleep, extractProductId, extractParentIdFromHtml, normalizeText, normalizeSku, parseVariantName, normalizeVariant, collectVariants, validateScanResult, buildSapoSkuIndex, diffInventory, summarize };
});
