...
assert.ok(retry.includes('yêu cầu SKU gốc website'));
assert.ok(retry.includes('lagRetryAttempted'));
assert.ok(retry.includes('lagRetryRecovered'));
assert.ok(retry.includes('variants.every(v=>String(v&&v.sku'));
...
console.log('CATALOG LAG RETRY PASS',{canonicalScanner:true,sourceSkuExact:true});
