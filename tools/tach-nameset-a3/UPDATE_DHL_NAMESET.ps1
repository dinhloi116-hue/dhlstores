...
try {
  Set-Content -LiteralPath $log -Value ('DHL Nameset Layout updater V8.19 - ' + (Get-Date)) -Encoding UTF8
  [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
  Log 'Downloading DHL Nameset Layout V8.19...'
  ...
  $raw = $raw.Replace('DHL_UI_VERSION=7.4','DHL_UI_VERSION=8.19').Replace('v7.4','v8.19')
  ...
  if ($verify.IndexOf('DHL_UI_VERSION=8.19') -lt 0 -or $verify.IndexOf('dhl-v819-easy-frame-ui') -lt 0) { throw 'Could not build V8.19 UI.' }
  Log ('Built V8.19 UI: ' + (Get-Item -LiteralPath $tmpOut).Length + ' bytes')
  ...
  foreach ($target in $uniq) {
    try {
      ...
      if ($check.IndexOf('DHL_UI_VERSION=8.19') -lt 0 -or $check.IndexOf('dhl-v819-easy-frame-ui') -lt 0) { throw 'Verification failed after write.' }
      $success++
      Log ('UPDATED V8.19: ' + $dst)
    } catch {
      $failed++
  ...
  Log ('DONE - V8.19 installed to ' + $success + ' location(s); failed/skipped: ' + $failed)
  Write-Host ''
  Write-Host 'DONE - DHL Nameset Layout V8.19 installed.' -ForegroundColor Cyan
  Write-Host 'V8.19: removes 60x100 preset, simplifies frame selection, adds section icons and inline ? help explanations.'
