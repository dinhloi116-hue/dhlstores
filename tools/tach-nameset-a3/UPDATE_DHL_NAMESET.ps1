...
try {
  Set-Content -LiteralPath $log -Value ('DHL Nameset Layout updater V8.18 - ' + (Get-Date)) -Encoding UTF8
  [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
  Log 'Downloading DHL Nameset Layout V8.18...'
  ...
  $raw = $raw.Replace('DHL_UI_VERSION=7.4','DHL_UI_VERSION=8.18').Replace('v7.4','v8.18')
  ...
  if ($verify.IndexOf('DHL_UI_VERSION=8.18') -lt 0 -or $verify.IndexOf('dhl-v818-frame-ui') -lt 0) { throw 'Could not build V8.18 UI.' }
  Log ('Built V8.18 UI: ' + (Get-Item -LiteralPath $tmpOut).Length + ' bytes')
  ...
  foreach ($target in $uniq) {
    try {
      ...
      if ($check.IndexOf('DHL_UI_VERSION=8.18') -lt 0 -or $check.IndexOf('dhl-v818-frame-ui') -lt 0) { throw 'Verification failed after write.' }
      $success++
      Log ('UPDATED V8.18: ' + $dst)
    } catch {
      $failed++
  ...
  Log ('DONE - V8.18 installed to ' + $success + ' location(s); failed/skipped: ' + $failed)
  Write-Host ''
  Write-Host 'DONE - DHL Nameset Layout V8.18 installed.' -ForegroundColor Cyan
  Write-Host 'V8.18: separates preset frame sizes from custom frame size controls while keeping spacing and margins as common settings.'
