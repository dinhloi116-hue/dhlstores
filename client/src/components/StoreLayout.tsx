...
import { ShoppingBag, User as UserIcon, ShieldCheck, Download, Package, Menu, X, LogOut, CheckCircle2, ChevronRight, Sparkles } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-purple-900 text-xs py-2 px-4 text-center font-medium tracking-wide text-slate-200 border-b border-slate-800 flex items-center justify-center gap-3">
        ...
        <span>Miễn phí vận chuyển áo đấu toàn quốc đơn từ 500k — Tải file số & font chữ tự động 24/7 ngay sau khi thanh toán</span>
        <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold">Physical & Digital Hub</p>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
            ...
            <Link href="/products?type=physical" className="transition-colors hover:text-amber-400 text-slate-300 flex items-center gap-1.5">
              <Package className="w-4 h-4 text-blue-400" /> Áo bóng đá
            </Link>
            <Link href="/products?type=digital" className="transition-colors hover:text-amber-400 text-slate-300 flex items-center gap-1.5">
              <Download className="w-4 h-4 text-purple-400" /> Sản phẩm số (File/Font)
            </Link>
          </nav>
          ...
          <div className="flex items-center gap-4">
            ...
            <Sheet open={cartOpen} onOpenChange={setCartOpen}>
              ...
              <SheetContent className="bg-slate-955 border-slate-800 text-slate-100 w-full sm:max-w-md flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto py-4 space-y-4">
                  {!isAuthenticated ? (
                    ...
                    <div className="text-center py-16 text-slate-400">
                      ...
                      <p className="text-xs text-slate-500 mt-1">Hãy khám phá các áo đấu và sản phẩm số tuyệt vời của DHL Stores nhé!</p>
                    </div>
                  ) : (
                    cartItems.map(item => {
            ...
            {isAuthenticated && user ? (
              <div className="flex items-center gap-3">
                <div className="hidden lg:block text-right">
                  <p className="text-xs font-semibold text-slate-200">{user.name || "Khách hàng"}</p>
                  <p className="text-[10px] text-amber-400 uppercase tracking-wider">{user.role === 'admin' ? 'Quản trị viên' : 'Thành viên'}</p>
                </div>
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl py-2 hidden group-hover:block group-focus-within:block z-50">
                    ...
                    <Link href="/account" className="flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-amber-400 transition-colors">
                      <Package className="w-4 h-4 text-blue-400" /> Đơn hàng & Tải tệp số
                    </Link>
                    {user.role === 'admin' && (
                      <Link href="/admin" className="flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-amber-400 transition-colors">
                        <ShieldCheck className="w-4 h-4 text-amber-400" /> Quản trị cửa hàng
                      </Link>
                    )}
                    <button
                      onClick={() => logout()}
              ...
              <Button
                onClick={() => startLogin()}
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-md px-5"
              >
                Đăng nhập
              </Button>
            )}
      ...
      <footer className="bg-slate-950 border-t border-slate-800/80 pt-16 pb-12 text-slate-400 text-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
          <div className="space-y-4">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Cửa hàng chuyên cung cấp áo bóng đá thiết kế đỉnh cao cùng các sản phẩm số chất lượng cao (file in hình ảnh 4K, font số độc quyền) phục vụ giới mộ điệu và nhà thiết kế chuyên nghiệp.
            </p>
          </div>

          <div>
            <h4 className="text-white font-semibold mb-4 text-xs uppercase tracking-wider">Danh mục sản phẩm</h4>
            <ul className="space-y-2.5 text-xs">
              <li><Link href="/products?type=physical" className="hover:text-amber-400 transition-colors">Áo bóng đá chính hãng</Link></li>
              <li><Link href="/products?type=digital" className="hover:text-amber-400 transition-colors">File in hình ảnh & Poster 4K</Link></li>
              <li><Link href="/products?type=digital" className="hover:text-amber-400 transition-colors">Font số áo đấu typography</Link></li>
              <li><Link href="/products" className="hover:text-amber-400 transition-colors">Tất cả sản phẩm mới</Link></li>
            </ul>
          </div>

          <div>
            <h4 className="text-white font-semibold mb-4 text-xs uppercase tracking-wider">Hỗ trợ khách hàng</h4>
            <ul className="space-y-2.5 text-xs">
              ...
              <li><span className="text-slate-300">Thời gian làm việc:</span> 08:00 - 22:00 (T2 - CN)</li>
              <li><span className="text-slate-300">Tải file số:</span> Tự động 24/7 sau khi thanh toán</li>
            </ul>
          </div>

          <div>
            <h4 className="text-white font-semibold mb-4 text-xs uppercase tracking-wider">Cam kết dịch vụ</h4>
            <div className="space-y-3 text-xs">
              <div className="flex items-center gap-2 text-slate-300">
                <ShieldCheck className="w-4 h-4 text-amber-400 flex-shrink-0" />
                <span>Bảo mật thanh toán & giao dịch tuyệt đối</span>
              </div>
              <div className="flex items-center gap-2 text-slate-300">
                <Download className="w-4 h-4 text-purple-400 flex-shrink-0" />
                <span>Link tải sản phẩm số tốc độ cao trọn đời</span>
              </div>
              <div className="flex items-center gap-2 text-slate-300">
                <Package className="w-4 h-4 text-blue-400 flex-shrink-0" />
                <span>Đổi trả dễ dàng cho sản phẩm áo đấu</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 border-t border-slate-900 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500">
          <p>© 2026 DHL Stores. Bản quyền thuộc về DHL Stores.</p>
          <p className="mt-2 sm:mt-0">Thiết kế thanh lịch, tinh tế và hoàn hảo từng chi tiết.</p>
        </div>
      </footer>
    </div>
  );
