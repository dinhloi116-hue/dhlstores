...
import { ShoppingBag, User as UserIcon, ShieldCheck, Download, Package, Menu, X, LogOut, Globe, Sparkles, WandSparkles, BookOpen, Trophy, Shirt, ChevronDown, WalletCards, MapPin, History, Bell, LoaderCircle, ArrowUp, Scale, Camera, Upload, Search, Film, Heart } from "lucide-react";
...
export default function StoreLayout({ children }: { children: React.ReactNode }) {
  ...
  const goToCheckout = () => {
    if (!cartItems.length || checkoutRedirecting) return;
    setCheckoutRedirecting(true);
    toast.info(lang === "vi" ? "Đang mở trang thanh toán…" : "Opening checkout…");
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 pb-16 text-slate-900 font-sans selection:bg-amber-400 selection:text-slate-950 md:pb-0">
                  ...
                  <div role="menu" className="overflow-hidden rounded-2xl border border-slate-200 bg-white py-2 shadow-2xl ring-1 ring-slate-950/5">
                    ...
                    <div className="grid grid-cols-2 gap-1 p-2">
                      <Link onClick={() => setAccountNavPending(true)} role="menuitem" href="/account#wallet" className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 hover:bg-amber-50 hover:text-amber-700"><WalletCards className="h-4 w-4 text-amber-600" />{lang === 'vi' ? 'Ví & số dư' : 'Wallet'}</Link>
	                      ...
	                      <Link onClick={() => setAccountNavPending(true)} role="menuitem" href="/saved" className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 hover:bg-rose-50 hover:text-rose-700"><Heart className="h-4 w-4 text-rose-500" />{lang === 'vi' ? 'Lưu mua sau' : 'Saved items'}</Link>
      ...
      {isAuthenticated && <div className="group/floating-cart fixed bottom-20 right-4 z-50 sm:right-6"><div className="pointer-events-none invisible absolute bottom-[calc(100%+0.75rem)] right-0 w-[min(22rem,calc(100vw-2rem))] translate-y-2 rounded-2xl border border-orange-200 bg-white p-3 opacity-0 shadow-2xl transition duration-200 group-hover/floating-cart:pointer-events-auto group-hover/floating-cart:visible group-hover/floating-cart:translate-y-0 group-hover/floating-cart:opacity-100 group-focus-within/floating-cart:pointer-events-auto group-focus-within/floating-cart:visible group-focus-within/floating-cart:translate-y-0 group-focus-within/floating-cart:opacity-100"><div className="flex items-center justify-between border-b border-slate-100 pb-2"><p className="text-xs font-black text-slate-900">Giỏ hàng của bạn</p><span className="text-[10px] font-black text-[#ee4d2d]">{cartItemCount} sản phẩm</span></div>{cartItems.length === 0 ? <div className="px-3 py-5 text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-orange-50 text-2xl">🛍️</div><p className="mt-2 text-sm font-black text-slate-800">Giỏ hàng đang trống</p><p className="mt-1 text-[11px] leading-relaxed text-slate-500">Khám phá tài nguyên và sản phẩm nổi bật của DHL Stores.</p><Link href="/products" onClick={() => setCartOpen(false)} className="mt-3 inline-flex items-center justify-center rounded-xl bg-[#ee4d2d] px-4 py-2 text-[11px] font-black text-white transition hover:bg-[#d94325]">Tiếp tục mua sắm</Link></div> : <><div className="mt-2 max-h-56 space-y-2 overflow-y-auto">{cartItems.slice(0, 4).map(item => <div key={item.id} className="flex items-center gap-2 rounded-xl bg-slate-50 p-2"><div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-white">{item.product?.image ? <img src={item.product.image} alt="" className="h-full w-full object-contain" /> : <ShoppingBag className="m-2 h-5 w-5 text-slate-300" />}</div><div className="min-w-0 flex-1"><p className="truncate text-[11px] font-black text-slate-800">{item.product?.name || "Sản phẩm"}</p><p className="text-[10px] text-slate-500">SL: {item.quantity}{item.variant?.sku ? ` · ${item.variant.sku}` : ""}</p><div className="mt-1 flex items-center gap-1"><button type="button" aria-label="Giảm số lượng" disabled={updateCartMutation.isPending} onClick={() => item.quantity <= 1 ? removeCartMutation.mutate({ cartItemId: item.id }) : updateCartMutation.mutate({ cartItemId: item.id, quantity: item.quantity - 1 })} className="grid h-6 w-6 place-items-center rounded border border-slate-200 bg-white text-xs font-black hover:bg-orange-50 disabled:opacity-50">−</button><span className="min-w-5 text-center text-[10px] font-black">{item.quantity}</span><button type="button" aria-label="Tăng số lượng" disabled={updateCartMutation.isPending} onClick={() => updateCartMutation.mutate({ cartItemId: item.id, quantity: item.quantity + 1 })} className="grid h-6 w-6 place-items-center rounded border border-slate-200 bg-white text-xs font-black hover:bg-orange-50 disabled:opacity-50">+</button><button type="button" aria-label="Xóa sản phẩm" disabled={removeCartMutation.isPending} onClick={() => removeCartMutation.mutate({ cartItemId: item.id })} className="ml-1 px-1 text-[10px] font-black text-rose-600 hover:text-rose-800 disabled:opacity-50">Xóa</button></div></div><span className="shrink-0 self-start text-[10px] font-black text-[#ee4d2d]">{formatCurrency((item.product ? parseFloat(item.product.price) + Number(item.variant?.priceAdjustment || 0) : 0) * item.quantity)}</span></div>)}{cartItems.length > 4 && <p className="text-center text-[10px] font-semibold text-slate-500">Còn {cartItems.length - 4} sản phẩm khác</p>}</div><div className="mt-3 border-t border-slate-100 pt-3"><div className="mb-3 rounded-xl bg-orange-50 px-3 py-2"><div className="flex items-center justify-between gap-2 text-[10px] font-bold text-orange-800"><span>{freeShippingRemaining > 0 ? `Mua thêm ${formatCurrency(freeShippingRemaining)} để được miễn phí vận chuyển` : "Bạn đã được miễn phí vận chuyển"}</span><span className="shrink-0">{freeShippingProgress}%</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-orange-100"><div className="h-full rounded-full bg-[#ee4d2d] transition-all duration-300" style={{ width: `${freeShippingProgress}%` }} /></div></div><div className="flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-wide text-slate-500">Tổng tạm tính</span><strong className="text-base font-black text-[#ee4d2d]">{formatCurrency(cartSubtotal)}</strong></div><div className="mt-2 grid grid-cols-2 gap-2"><Link href="/checkout" onClick={() => setCartOpen(false)} className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-2 py-2 text-[10px] font-black text-slate-700 transition hover:border-orange-300 hover:bg-orange-50">Xem chi tiết giỏ hàng</Link><button type="button" disabled={checkoutRedirecting} onClick={goToCheckout} className="inline-flex items-center justify-center gap-1 rounded-xl bg-[#ee4d2d] px-2 py-2 text-[10px] font-black text-white transition hover:bg-[#d94325] disabled:cursor-wait disabled:opacity-70">{checkoutRedirecting && <LoaderCircle className="h-3.5 w-3.5 animate-spin" />}{checkoutRedirecting ? "Đang mở…" : "Thanh toán ngay"}</button></div></div></>}</div><button type="button" id="floating-cart-trigger" onClick={() => setCartOpen(true)} aria-label={`${lang === "vi" ? "Mở giỏ hàng, " : "Open cart, "}${cartItemCount} ${lang === "vi" ? "sản phẩm" : "items"}`} className={`grid h-14 w-14 place-items-center rounded-full border-4 border-white bg-[#ee4d2d] text-white shadow-2xl transition hover:-translate-y-1 hover:bg-[#d94325] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-300 ${cartPulse ? "animate-[bounce_0.5s_ease-in-out]" : ""}`}><ShoppingBag className="h-6 w-6" /><span className="absolute -right-1 -top-1 grid min-h-6 min-w-6 place-items-center rounded-full border-2 border-white bg-amber-400 px-1 text-[10px] font-black text-slate-950">{cartItemCount}</span></button></div>}
