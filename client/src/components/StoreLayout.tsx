...
import { ShoppingBag, User as UserIcon, ShieldCheck, Download, Package, Menu, X, LogOut, Globe, Sparkles, WandSparkles, BookOpen, Trophy, Shirt, ChevronDown, WalletCards, MapPin, History, Bell, LoaderCircle, ArrowUp, Scale, Camera, Upload, Search } from "lucide-react";
        ...
        <div className="mx-auto flex h-16 max-w-[1600px] items-center justify-between px-3 sm:h-[4.5rem] sm:px-6 lg:px-8 2xl:px-10">
          ...
          <nav className="hidden md:flex items-center gap-3 text-sm font-semibold">
            ...
            <div className="group relative">
              ...
              <div className="pointer-events-none invisible absolute left-1/2 top-full z-[70] w-[min(22rem,calc(100vw-2rem))] -translate-x-1/2 pt-3 opacity-0 transition-[opacity,visibility] duration-150 group-hover:pointer-events-auto group-hover:visible group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:visible group-focus-within:opacity-100"><div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl"><div className="mb-2 flex items-center justify-between gap-3 px-1"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">DHL Stores · Công cụ</p><p className="mt-1 text-sm font-black text-slate-900">Chọn tiện ích nhanh</p></div><Link href="/tools" className="text-[11px] font-black text-violet-700 hover:text-violet-900">Xem thư viện →</Link></div><div className="grid gap-2"><Link href="/tools/notebook-labels" className="group/tool rounded-xl border border-amber-200 bg-gradient-to-r from-amber-50 to-violet-50 p-3 transition hover:-translate-y-0.5 hover:border-violet-400"><div className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-amber-400 text-slate-950"><BookOpen className="h-5 w-5" /></span><span className="min-w-0"><strong className="block text-sm font-black text-slate-900">Tạo Nhãn Vở Online</strong><span className="mt-0.5 block text-[11px] leading-relaxed text-slate-500">Nhập Excel, dàn nhãn A4 và xuất PDF để in.</span></span><ArrowUp className="ml-auto h-4 w-4 rotate-45 text-violet-600 transition group-hover/tool:translate-x-0.5" /></div></Link><a href="/manus-storage/pet-tram-pro-x_89dce948.html" className="rounded-xl border border-cyan-200 bg-slate-950 p-3 text-white transition hover:-translate-y-0.5 hover:border-cyan-400"><strong className="block text-sm font-black">PET TRAM PRO X</strong><span className="mt-0.5 block text-[11px] leading-relaxed text-slate-300">Xử lý ảnh, tạo tram, vector hóa và xuất file in.</span></a></div></div></div>
            </div>
          </nav>
