...
export default function ProductDetail() {
  ...
  const [variantSort, setVariantSort] = useState<'label' | 'price-asc' | 'price-desc' | 'stock-desc'>('label');
            ...
            {product.type === "physical" && variants.length > 0 && (
              <section className="space-y-4 rounded-xl border border-emerald-200 bg-emerald-50/60 p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  ...
                  <label className="text-xs font-bold text-slate-700">Sắp xếp SKU<select value={variantSort} onChange={event => setVariantSort(event.target.value as typeof variantSort)} className="mt-1 block h-9 w-full rounded-lg border border-emerald-200 bg-white px-2 text-xs font-bold text-slate-800"><option value="label">Tên / kiểu A–Z</option><option value="stock-desc">Tồn kho cao → thấp</option><option value="price-asc">Giá thấp → cao</option><option value="price-desc">Giá cao → thấp</option></select></label>
                </div>
