                ...
                <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="max-h-80 overflow-y-auto">
                  ...
                  {sortedVariants.map(variant => {
                    ...
                    return <button key={variant.id} type="button" disabled={outOfStock} onClick={() => setSelectedVariantId(variant.id)} className={`grid w-full grid-cols-[3.5rem_minmax(0,1fr)_minmax(10.5rem,auto)] items-center gap-3 border-t border-slate-100 px-3 py-2 text-left text-xs transition-colors ${isSelected ? "bg-emerald-50" : "hover:bg-slate-50"} disabled:cursor-not-allowed disabled:opacity-55`}>
                      ...
                      <span className="flex min-w-[10.5rem] items-center justify-end gap-1.5 whitespace-nowrap"><span className="inline-flex max-w-[6.75rem] items-center gap-1 rounded-md bg-slate-100 px-1.5 py-1 font-mono text-[10px] text-slate-600" title={variant.sku || "Không có SKU"}><span className="font-sans text-[8px] font-black uppercase text-slate-400">SKU</span><span className="truncate">{variant.sku || "—"}</span></span><span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-black ${outOfStock ? "bg-rose-100 text-rose-700" : variant.stock <= 5 ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}><span className="text-[8px] uppercase opacity-70">Tồn</span>{outOfStock ? "Hết" : variant.stock}</span></span>
                    </button>;
                  })}
                </div></div>
              </section>
