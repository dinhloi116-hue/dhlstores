...
export default function ProductDetail() {
  ...
  const skuInventoryPanel = product.type === "physical" && variants.length > 0 ? (
    <aside className="min-w-0 rounded-xl border border-slate-200 bg-slate-50/80 p-3 xl:sticky xl:top-4">
      ...
      <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="max-h-[26rem] overflow-y-auto">
        ...
        {sortedVariants.map(variant => {
          ...
          return <button key={variant.id} type="button" aria-disabled={outOfStock} onClick={() => { if (!outOfStock) setSelectedVariantId(variant.id); }} className={`grid w-full grid-cols-[3.5rem_minmax(0,1fr)_minmax(10.5rem,auto)] items-center gap-3 border-t border-slate-100 px-3 py-2 text-left text-xs transition-colors ${isSelected ? "bg-emerald-50" : "hover:bg-slate-50"} ${outOfStock ? "cursor-not-allowed opacity-55" : ""}`}>
            <span aria-label={variant.image ? `Xem ảnh lớn ${formatVariantOptions(variant)}` : undefined} onMouseEnter={event => { if (variant.image && window.innerWidth >= 768) updateHoveredPreview(variant.id, event.clientX, event.clientY); }} onMouseMove={event => { if (variant.image && window.innerWidth >= 768) updateHoveredPreview(variant.id, event.clientX, event.clientY); }} onMouseLeave={() => setHoveredPreview(null)} onClick={event => { if (variant.image) { event.stopPropagation(); setPreviewVariantId(variant.id); } }} className={`h-11 w-11 overflow-hidden rounded-md border border-slate-200 bg-slate-50 ${variant.image ? "cursor-zoom-in focus:outline-none focus:ring-2 focus:ring-cyan-500" : ""}`}>{variant.image ? <img src={variant.image} alt={formatVariantOptions(variant)} className="h-full w-full object-contain" /> : <span className="flex h-full items-center justify-center text-[9px] font-bold text-slate-400">SKU</span>}</span>
