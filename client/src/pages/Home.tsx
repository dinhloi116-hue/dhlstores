  ...
  return (
    <StoreLayout>
      ...
      <section className="border-y border-slate-200 bg-slate-50/70 py-8">
        <div className="mx-auto grid max-w-7xl gap-5 px-4 sm:px-6 lg:grid-cols-[1.15fr_.85fr] lg:px-8">
          ...
          <div className="rounded-2xl bg-slate-950 p-5 text-white shadow-sm"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-400">DHL Stores · cách mua</p><h2 className="mt-1 font-display text-2xl font-black uppercase leading-tight">Chọn nhanh, thanh toán rõ, nhận đúng sản phẩm</h2><div className="mt-4 space-y-3">{[{ icon: PackageCheck, title: "1. Chọn sản phẩm", copy: "Chọn tài nguyên hoặc phiên bản hàng vật lý phù hợp." }, { icon: ShieldCheck, title: "2. Quét QR Techcombank", copy: "QR hiện ngay tại trang sản phẩm; chỉ cần chuyển đúng số tiền, không cần cú pháp." }, { icon: Download, title: "3. Chủ cửa hàng xác nhận", copy: "Sau khi tiền về được kiểm tra, tài nguyên số mở tải ngay; hàng vật lý được theo dõi theo đơn." }].map(step => <div key={step.title} className="flex gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/10 text-amber-400"><step.icon className="h-4 w-4" /></div><div><p className="text-xs font-black text-white">{step.title}</p><p className="mt-0.5 text-[11px] leading-relaxed text-slate-300">{step.copy}</p></div></div>)}</div></div>
        </div>
      </section>
