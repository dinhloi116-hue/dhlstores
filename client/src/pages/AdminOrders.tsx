      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="products" className="space-y-6">
            ...
            <div className="grid gap-6 xl:grid-cols-3 xl:[&>div:nth-of-type(1)]:order-1 xl:[&>section]:order-2 xl:[&>div:nth-of-type(2)]:order-3">
              <section className="overflow-hidden rounded-2xl border border-violet-200 bg-white shadow-sm">
                <div className="flex items-start justify-between gap-3 border-b border-violet-100 bg-violet-50/70 p-5">
                  <div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-700">Cột 2 · SKU & tồn kho</p><h2 className="font-display text-2xl font-black uppercase text-slate-900">{selectedProduct?.name || "Chọn sản phẩm"}</h2><p className="mt-1 text-xs leading-relaxed text-slate-600">Kéo thả hoặc dùng mũi tên để đặt thứ tự SKU hiển thị cố định cho khách.</p></div>
                  {selectedProduct && <Badge className={selectedProduct.type === "physical" ? "bg-violet-100 text-violet-800" : "bg-blue-100 text-blue-800"}>{selectedProduct.type === "physical" ? `${variants.length} SKU` : "Tài nguyên số"}</Badge>}
                </div>
                {!selectedProduct ? <div className="p-8 text-center text-sm leading-relaxed text-slate-500">Chọn một sản phẩm ở cột bên trái để xem hình SKU, tồn kho và biểu mẫu chỉnh sửa.</div> : selectedProduct.type !== "physical" ? <div className="p-8 text-center text-sm leading-relaxed text-slate-500">Sản phẩm số không dùng SKU vật lý. Bạn có thể cập nhật ảnh, tệp và liên kết tải ở biểu mẫu bên phải.</div> : <div className="max-h-[48rem] divide-y divide-slate-100 overflow-y-auto">{variants.map((variant, index) => <article key={variant.id} draggable onDragStart={() => setDraggedVariantId(variant.id)} onDragEnd={() => setDraggedVariantId(null)} onDragOver={event => event.preventDefault()} onDrop={() => { if (!draggedVariantId || draggedVariantId === variant.id) return; const ids = variants.map(item => item.id); const fromIndex = ids.indexOf(draggedVariantId); const toIndex = ids.indexOf(variant.id); ids.splice(fromIndex, 1); ids.splice(toIndex, 0, draggedVariantId); saveVariantOrder(ids); setDraggedVariantId(null); }} className={`group flex cursor-grab gap-3 p-3 transition active:cursor-grabbing ${editingVariantId === variant.id ? "bg-violet-50" : "hover:bg-slate-50"}`}>
                  <div className="flex h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-100">{variant.image || selectedProduct.image ? <img src={variant.image || selectedProduct.image} alt={`Ảnh SKU ${variant.sku || index + 1}`} className="h-full w-full object-cover" /> : <div className="grid h-full w-full place-items-center text-[10px] font-black text-slate-400">SKU</div>}</div>
                  <button type="button" onClick={() => { setEditingVariantId(variant.id); setVariantDraft({ size: variant.size || "", color: variant.color || "", attributes: variant.attributes || "", sku: variant.sku || "", image: variant.image || "", priceAdjustment: variant.priceAdjustment, stock: String(variant.stock), isActive: variant.isActive }); }} className="min-w-0 flex-1 text-left"><p className="truncate text-sm font-black text-slate-900">{[variant.color, variant.size].filter(Boolean).join(" · ") || `SKU ${index + 1}`}</p><p className="mt-1 truncate font-mono text-[11px] text-slate-500">{variant.sku || "Chưa có mã SKU"}</p><div className="mt-2 flex flex-wrap gap-1.5"><Badge className={variant.stock <= 5 ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"}>Tồn {variant.stock}</Badge><Badge className={variant.isActive ? "bg-slate-100 text-slate-700" : "bg-slate-200 text-slate-500"}>{variant.isActive ? "Đang bán" : "Đang ẩn"}</Badge></div></button>
                  <div className="flex shrink-0 flex-col gap-1 opacity-100 xl:opacity-0 xl:transition-opacity xl:group-hover:opacity-100"><Button type="button" size="icon" variant="ghost" className="h-7 w-7" onClick={() => moveVariant(variant.id, -1)} disabled={index === 0 || reorderProductVariants.isPending} aria-label="Đưa SKU lên"><ArrowUp className="h-3.5 w-3.5" /></Button><Button type="button" size="icon" variant="ghost" className="h-7 w-7" onClick={() => moveVariant(variant.id, 1)} disabled={index === variants.length - 1 || reorderProductVariants.isPending} aria-label="Đưa SKU xuống"><ArrowDown className="h-3.5 w-3.5" /></Button></div>
                </article>)}{variants.length === 0 && <div className="p-8 text-center text-sm text-slate-500">Chưa có SKU. Dùng tab Biến thể để tạo lựa chọn đầu tiên.</div>}</div>}
              </section>
