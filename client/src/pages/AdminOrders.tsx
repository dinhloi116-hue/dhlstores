...
import { ShieldCheck, Package, ArrowLeft, CheckCircle2, Clock, Truck, XCircle, Users, DollarSign } from "lucide-react";
    ...
    return (
      <StoreLayout>
        <div className="max-w-7xl mx-auto px-4 py-24 text-center space-y-4">
          ...
          <p className="text-slate-400 text-xs">Bạn cần có quyền quản trị viên (Admin) để truy cập trang này.</p>
          <Link href="/">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
        ...
        <div className="bg-slate-900/80 p-8 rounded-3xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-white">Quản Trị Cửa Hàng DHL Stores</h1>
              <p className="text-xs text-slate-400 mt-1">Quản lý toàn bộ đơn hàng vật lý và sản phẩm số của khách hàng.</p>
            </div>
          </div>
          <Link href="/account">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-900/80 p-6 rounded-2xl border border-slate-800 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-slate-400 uppercase font-semibold">Tổng Doanh Thu</p>
          ...
          <div className="bg-slate-900/80 p-6 rounded-2xl border border-slate-800 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-slate-400 uppercase font-semibold">Tổng Đơn Hàng</p>
              <p className="text-xl font-black text-white mt-1">{orders.length} đơn</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900/80 p-6 rounded-2xl border border-slate-800 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-slate-400 uppercase font-semibold">Phân Loại Đơn</p>
              <p className="text-xs font-semibold text-slate-300 mt-1">Áo đấu: {physicalOrdersCount} | Sản phẩm số: {digitalOrdersCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <h2 className="text-xl font-black text-white">Danh Sách Đơn Hàng Cần Xử Lý</h2>
                ...
                <div key={order.id} className="bg-slate-900/80 rounded-2xl border border-slate-800 overflow-hidden shadow-xl">
                  ...
                  <div className="bg-slate-950/80 px-6 py-4 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-slate-400">Khách ID: {order.userId}</span>
                      <span className="text-slate-400">Ngày: {new Date(order.createdAt).toLocaleString('vi-VN')}</span>
                    </div>
                      ...
                      return (
                        <div key={item.id} className="py-3 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-white">{p?.name || "Sản phẩm"}</p>
                              {item.attributes && <p className="text-xs text-amber-400">{item.attributes}</p>}
                              <p className="text-xs text-slate-400">SL: {item.quantity} x {formatCurrency(Number(item.price))}</p>
                            </div>
                          </div>
                          <Badge className={p?.type === 'physical' ? 'bg-blue-600/20 text-blue-400' : 'bg-purple-600/20 text-purple-400'}>
                            {p?.type === 'physical' ? 'Áo đấu vật lý' : 'Sản phẩm số'}
                          </Badge>
                        </div>
                      );
                    })}
                  ...
                  {order.shippingAddress && (
                    <div className="bg-blue-950/20 px-6 py-3 border-t border-slate-800 text-xs flex flex-col sm:flex-row justify-between gap-2 text-slate-300">
                      <div>
                        <strong className="text-white">Địa chỉ giao hàng:</strong> {order.shippingName} ({order.shippingPhone}) - {order.shippingAddress}
                      </div>
                      {order.shippingNote && (
                        <div><strong className="text-white">Ghi chú:</strong> {order.shippingNote}</div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="bg-slate-950/40 px-6 py-4 border-t border-slate-800 flex items-center justify-between text-sm">
                    <span className="text-slate-400">Tổng doanh thu đơn:</span>
