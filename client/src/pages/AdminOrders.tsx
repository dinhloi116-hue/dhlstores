...
type VariantDraft = { size: string; color: string; attributes: string; sku: string; image: string; priceAdjustment: string; stock: string; weightGrams?: string; isActive: boolean };
...
const emptyProduct: ProductDraft = {
  name: "", slug: "", description: "", price: "", categoryId: 0, image: "generated:catalog-cover", fileUrl: "", fileSize: "", specs: "", stock: "0", weightGrams: "0", featured: false, isActive: true,
};
const emptyVariant: VariantDraft = { size: "", color: "", attributes: "", sku: "", image: "", priceAdjustment: "0", stock: "0", weightGrams: "", isActive: true };
...
export default function AdminOrders() {
  ...
  useEffect(() => {
    ...
    if (variant) {
      ...
      setVariantDraft({ size: variant.size || "", color: variant.color || "", attributes: variant.attributes || "", sku: variant.sku || "", image: variant.image || "", priceAdjustment: variant.priceAdjustment, stock: String(variant.stock), weightGrams: variant.weightGrams === undefined ? "" : String(variant.weightGrams), isActive: variant.isActive });
    }
  }, [location, variants]);
  ...
  const handleVariantSubmit = (event: FormEvent) => {
    ...
    const data = { ...variantDraft, productId: selectedVariantProductId, priceAdjustment: Number(variantDraft.priceAdjustment || 0), stock: Number(variantDraft.stock || 0), weightGrams: !variantDraft.weightGrams?.trim() ? undefined : Number(variantDraft.weightGrams) };
    if (editingVariantId) updateVariant.mutate({ variantId: editingVariantId, data: { size: data.size || undefined, color: data.color || undefined, attributes: data.attributes || undefined, sku: data.sku || undefined, image: data.image || undefined, priceAdjustment: data.priceAdjustment, stock: data.stock, weightGrams: data.weightGrams, isActive: data.isActive } });
  ...
  const saveSkuQuick = (variantId: number) => {
    ...
    if (!draft || Number(draft.stock) < 0 || !Number.isFinite(Number(draft.priceAdjustment))) return toast.error("Giá hoặc tồn kho SKU không hợp lệ");
    bulkUpdateProductVariants.mutate({ productId: selectedVariantProductId, changes: [{ variantId, stock: Number(draft.stock), priceAdjustment: Number(draft.priceAdjustment), isActive: draft.isActive }] });
  };
  ...
  const applyBulkSkuValues = () => {
    ...
    if (stock === undefined && priceAdjustment === undefined) return toast.error("Nhập giá hoặc tồn kho muốn áp dụng");
    if ((stock !== undefined && (!Number.isInteger(stock) || stock < 0)) || (priceAdjustment !== undefined && !Number.isFinite(priceAdjustment))) return toast.error("Giá hoặc tồn kho hàng loạt không hợp lệ");
    bulkUpdateProductVariants.mutate({ productId: selectedVariantProductId, changes: selectedSkuIds.map(variantId => ({ variantId, ...(stock === undefined ? {} : { stock }), ...(priceAdjustment === undefined ? {} : { priceAdjustment }) })) });
  };
