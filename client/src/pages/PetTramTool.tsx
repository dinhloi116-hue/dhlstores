      ...
      <main className="mx-auto max-w-[1600px] px-3 py-5 sm:px-6 lg:px-8">
        <section className="mb-4 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-relaxed text-slate-500">Xử lý ảnh, tạo tram, vector hóa, xem mockup áo và xuất hàng loạt. Chạy toàn màn hình giúp thao tác chọn tệp, popup và tải xuống ổn định hơn; ảnh bạn chọn chỉ được xử lý trong phiên trình duyệt của bạn.</p>
          </div>
          <div className="flex flex-wrap gap-2"><span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800"><ShieldCheck className="h-4 w-4" />Xử lý cục bộ</span><a href={TOOL_URL}><Button size="sm" className="bg-cyan-700 text-white hover:bg-cyan-600"><ExternalLink className="mr-1.5 h-4 w-4" />Chạy toàn màn hình</Button></a><a href={TOOL_URL} target="_blank" rel="noreferrer"><Button variant="outline" size="sm" className="border-cyan-200 text-cyan-800 hover:bg-cyan-50">Mở tab mới</Button></a></div>
        </section>
        <section className="overflow-hidden rounded-2xl border border-slate-300 bg-slate-950 shadow-xl">
          <iframe title="PET TRAM PRO X" src={TOOL_URL} sandbox="allow-scripts allow-forms allow-downloads allow-popups allow-popups-to-escape-sandbox allow-modals" className="h-[calc(100vh-14rem)] min-h-[700px] w-full bg-slate-950" />
        </section>
