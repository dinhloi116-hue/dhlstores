...
import { Download, Search, Filter, Eye, ShoppingCart, Zap } from "lucide-react";
...
export default function Products() {
  ...
  const previewPrice = (product: (typeof products)[number]) => lowestWholesalePriceByProduct.get(product.id) || Number(product.price);
  const hasWholesalePreviewPrice = (product: (typeof products)[number]) => lowestWholesalePriceByProduct.has(product.id);
    ...
    <StoreLayout>
            ...
            {products.map((p, index) => (
              ...
              <Link href={`/product/${p.slug}`} className="block h-full">
                  ...
                  <div className="group/media relative aspect-square overflow-hidden bg-[#0b1220] [background-image:linear-gradient(rgba(255,255,255,.06)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.06)_1px,transparent_1px)] [background-size:22px_22px]">
                    ...
                    <img src={p.image} alt={catalogName(p, lang)} loading={index < 4 ? "eager" : "lazy"} decoding="async" onError={event => { event.currentTarget.style.display = 'none'; }} className={`relative z-10 h-full w-full transition-transform duration-300 group-hover/media:scale-105 ${isPhysicalCatalog ? 'object-cover' : 'object-contain'}`} />
                  ...
                  <div className={`flex flex-1 flex-col justify-between ${isPhysicalCatalog ? 'space-y-2 p-2.5 sm:p-4' : 'space-y-2 p-2.5 sm:space-y-3 sm:p-4'}`}>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                      <span className={`${isPhysicalCatalog ? 'text-base text-[#d94325]' : 'text-sm text-amber-600'} font-black`}>
                        {isPhysicalCatalog && hasWholesalePreviewPrice(p) && <span className="mr-1 text-[10px] font-bold text-[#c2410c]"><Zap className="mr-0.5 inline-block h-3 w-3 animate-[pulse_1.8s_ease-in-out_infinite] align-[-1px] text-[#f59e0b]" aria-hidden="true" />{lang === "vi" ? "Giá sỉ từ" : "Wholesale from"}</span>}{formatCurrency(isPhysicalCatalog ? previewPrice(p) : p.price)}
                      </span>
              ...
              <div className="absolute right-2 top-2 z-30 flex items-center gap-1 opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100"><button type="button" onClick={() => setQuickViewProduct(p)} aria-label={`${lang === 'vi' ? 'Xem nhanh' : 'Quick view'} ${catalogName(p, lang)}`} className="inline-flex h-8 items-center gap-1 rounded-lg border border-white/80 bg-white/95 px-2 text-[10px] font-black text-slate-800 shadow-sm transition hover:bg-amber-400 hover:text-slate-950 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400"><Eye className="h-3.5 w-3.5" /><span className="hidden sm:inline">{lang === 'vi' ? 'Xem nhanh' : 'Quick view'}</span></button>{(p.type === 'digital' || Number(p.stock) > 0) && <button type="button" disabled={addToCartMutation.isPending} onClick={() => handleQuickAdd(p)} aria-label={`${lang === 'vi' ? 'Thêm nhanh' : 'Quick add'} ${catalogName(p, lang)}`} className="inline-flex h-8 items-center gap-1 rounded-lg bg-amber-500 px-2 text-[10px] font-black text-slate-950 shadow-sm transition hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-50"><ShoppingCart className="h-3.5 w-3.5" /><span className="hidden sm:inline">{lang === 'vi' ? 'Thêm nhanh' : 'Quick add'}</span></button>}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <Dialog open={Boolean(quickViewProduct)} onOpenChange={open => { if (!open) setQuickViewProduct(null); }}>
        <DialogContent className="max-w-2xl overflow-hidden border-slate-200 bg-white p-0 text-slate-900">
          {quickViewProduct && <div className="grid sm:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]"><div className="aspect-square bg-slate-50 p-4"><img src={quickViewProduct.image} alt={catalogName(quickViewProduct, lang)} className="h-full w-full object-contain" /></div><div className="flex flex-col p-6"><DialogHeader><Badge className={`w-fit ${quickViewProduct.type === "physical" ? "bg-orange-100 text-orange-800" : "bg-violet-100 text-violet-800"}`}>{quickViewProduct.type === "physical" ? (lang === 'vi' ? 'Hàng vật lý' : 'Physical item') : (lang === 'vi' ? 'Tài nguyên số' : 'Digital asset')}</Badge><DialogTitle className="mt-3 text-left font-display text-3xl font-black uppercase leading-none text-slate-900">{catalogName(quickViewProduct, lang)}</DialogTitle><DialogDescription className="mt-3 text-left text-xs leading-relaxed text-slate-600">{catalogDescription(quickViewProduct, lang) || (lang === 'vi' ? 'Mở chi tiết để xem toàn bộ thông tin sản phẩm.' : 'Open details to view the full product information.')}</DialogDescription></DialogHeader><div className="mt-5 space-y-3"><p className="text-2xl font-black text-[#d94325]">{quickViewProduct.type === "physical" && hasWholesalePreviewPrice(quickViewProduct) && <span className="mr-1 text-sm font-bold text-[#c2410c]"><Zap className="mr-0.5 inline-block h-3 w-3 animate-[pulse_1.8s_ease-in-out_infinite] align-[-1px] text-[#f59e0b]" aria-hidden="true" />{lang === "vi" ? "Giá sỉ từ" : "Wholesale from"}</span>}{formatCurrency(quickViewProduct.type === "physical" ? previewPrice(quickViewProduct) : quickViewProduct.price)}</p>{quickViewProduct.type === "physical" ? <div className={`rounded-xl border p-3 text-xs ${Number(quickViewProduct.stock) > 0 ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-rose-200 bg-rose-50 text-rose-800"}`}><p className="font-black">{Number(quickViewProduct.stock) > 0 ? (lang === 'vi' ? `Tồn kho tóm tắt: ${quickViewProduct.stock}` : `Stock summary: ${quickViewProduct.stock}`) : (lang === 'vi' ? 'Sản phẩm đang hết hàng' : 'This product is out of stock')}</p><p className="mt-1 leading-relaxed">{lang === 'vi' ? 'Mở chi tiết để kiểm tra tồn, màu, size và giá theo từng SKU thực tế.' : 'Open details to check live stock, colour, size and price for each SKU.'}</p></div> : <div className="rounded-xl border border-violet-200 bg-violet-50 p-3 text-xs leading-relaxed text-violet-900"><p className="font-black">{lang === 'vi' ? 'Tài nguyên số' : 'Digital asset'}</p><p className="mt-1">{lang === 'vi' ? 'Liên kết tải được mở sau khi giao dịch được xác nhận.' : 'Your download link becomes available once payment is confirmed.'}</p></div>}</div><Link href={`/product/${quickViewProduct.slug}`} onClick={() => setQuickViewProduct(null)} className="mt-auto inline-flex justify-center rounded-xl bg-slate-900 px-4 py-3 text-sm font-black text-white transition hover:bg-amber-500 hover:text-slate-950">{lang === 'vi' ? 'Xem chi tiết & chọn SKU' : 'View details & choose SKU'}</Link></div></div>}
        </DialogContent>
      </Dialog>
    </StoreLayout>
  );
