...
import { BookOpen, Download, FileImage, FileSpreadsheet, FileText, Image as ImageIcon, Loader2, Palette, Plus, Printer, Trash2, Type, Upload, WandSparkles } from "lucide-react";
...
async function drawLabel(row: LabelRow, template: LabelTemplate, illustrationUrl?: string, fontFamily = "Arial", textColor = "#172033", width = 1000, height = 540) {
...
function LabelPreview({ row, template, illustrationUrl, fontFamily, textColor, index, onSelect }: { row: LabelRow; template: LabelTemplate; illustrationUrl?: string; fontFamily: string; textColor: string; index: number; onSelect: () => void }) {
  const [src, setSrc] = useState("");
  useEffect(() => { let active = true; void drawLabel(row, template, illustrationUrl, fontFamily, textColor, 600, 324).then((value) => { if (active) setSrc(value); }); return () => { active = false; }; }, [row, template, illustrationUrl, fontFamily, textColor]);
...
export default function NotebookLabelMaker() {
  ...
  const activeIllustrationUrl = customImage || illustration.url;
  ...
  const importCustomImage = (file: File) => {
    if (!file.type.startsWith("image/")) return toast.error("Hãy chọn tệp ảnh PNG, JPG hoặc WebP.");
    if (file.size > 4 * 1024 * 1024) return toast.error("Ảnh cá nhân tối đa 4 MB.");
    const reader = new FileReader();
    reader.onload = () => { setCustomImage(String(reader.result)); toast.success("Đã thêm ảnh cá nhân vào nhãn."); };
    reader.onerror = () => toast.error("Không đọc được ảnh. Hãy thử tệp khác.");
    reader.readAsDataURL(file);
  };

  const downloadCurrentImage = async () => {
    try {
      const image = await drawLabel(activeRow, template, activeIllustrationUrl, fontFamily, textColor, 1200, 648);
      const link = document.createElement("a"); link.href = image; link.download = `nhan-vo-${activeIndex + 1}.png`; link.click();
      toast.success("Đã tải nhãn hiện tại dạng PNG.");
    } catch { toast.error("Không thể tạo ảnh nhãn trên trình duyệt này."); }
  };
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      for (let index = 0; index < rows.length; index += 1) {
        ...
        pdf.addImage(await drawLabel(row, template, activeIllustrationUrl, fontFamily, textColor), "PNG", x, y, cellW, cellH, undefined, "FAST");
      }
  ...
  return <StoreLayout><main className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8"><section className="relative overflow-hidden rounded-3xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-violet-50 p-6 shadow-sm sm:p-9"><div className="absolute -right-20 -top-24 h-64 w-64 rounded-full bg-violet-200/40 blur-3xl" /><div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><Badge className="bg-amber-100 text-amber-900"><BookOpen className="mr-1 h-3.5 w-3.5" />DHL Stores · Công cụ</Badge><h1 className="mt-4 font-display text-4xl font-black uppercase leading-none text-slate-950 sm:text-6xl">Tạo nhãn vở online</h1><p className="mt-4 max-w-3xl text-sm leading-relaxed text-slate-600 sm:text-base">Nhập thông tin một học sinh hoặc tải danh sách Excel, chọn mẫu nhãn, dàn trang sẵn theo A4 rồi tải PDF để in.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => customImageRef.current?.click()}><FileImage className="mr-2 h-4 w-4" />Thêm ảnh</Button><Button variant="outline" onClick={() => fileRef.current?.click()}><Upload className="mr-2 h-4 w-4" />Nhập Excel</Button><Button variant="outline" onClick={() => void downloadCurrentImage()}><Download className="mr-2 h-4 w-4" />Tải PNG</Button><Button onClick={exportPdf} disabled={isExporting}>{isExporting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}Xuất PDF</Button><input ref={customImageRef} type="file" hidden accept="image/png,image/jpeg,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) importCustomImage(file); event.currentTarget.value = ""; }} /><input ref={fileRef} type="file" hidden accept=".xlsx,.xls,.csv" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importSpreadsheet(file); event.currentTarget.value = ""; }} /></div></div></section>
