        ...
        <main className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
          <section className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
            <div className="px-6 py-5 bg-slate-900 text-white">
              <p className="text-[11px] tracking-[0.18em] font-bold uppercase text-amber-300">DHL Stores · Techcombank QR</p>
              ...
              <div className="p-6 sm:p-8 grid gap-7 sm:grid-cols-[1fr_260px] items-center">
                <div className="space-y-5">
                  ...
                  <div className="rounded-xl border border-cyan-200 bg-cyan-50 p-4">
                    <p className="text-xs font-black text-cyan-900">{lang === "vi" ? "Không cần nội dung chuyển khoản" : "No transfer memo required"}</p>
                    <p className="mt-2 text-[11px] leading-relaxed text-cyan-800">{lang === "vi" ? "Chỉ cần chuyển đúng số tiền. Chủ cửa hàng sẽ kiểm tra giao dịch Techcombank và xác nhận đơn trực tiếp." : "Transfer the displayed amount only. The store owner will verify the Techcombank transaction and confirm the order."}</p>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-slate-500"><span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />{lang === "vi" ? "Đang chờ chủ cửa hàng xác nhận thanh toán… QR hết hạn sau 10 phút." : "Waiting for the store owner to confirm payment… QR expires in 10 minutes."}</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-center">
                  {pendingPayment.qrUrl ? <img src={pendingPayment.qrUrl} alt="Mã QR Techcombank DHL Stores" className="w-full rounded-xl bg-white" /> : <div className="aspect-square grid place-items-center rounded-xl border border-dashed border-slate-300 bg-white p-6"><div><QrCode className="w-12 h-12 mx-auto text-slate-400" /><p className="text-xs text-slate-500 mt-3">{lang === "vi" ? "QR sẽ hiển thị khi cấu hình Techcombank hoàn tất." : "The QR will appear after Techcombank configuration is complete."}</p></div></div>}
