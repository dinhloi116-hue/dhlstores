...
export default function VideoCutterJoiner() {
  ...
  const runNormalize = async () => {
    ...
    try {
      ...
      const normalizedFile = new File([blob], `${active.file.name.replace(/\.[^.]+$/, "")}-converted.mp4`, { type: "video/mp4" });
      const normalizedUrl = URL.createObjectURL(normalizedFile);
      setVideos((current) => current.map((item) => {
        if (item.id !== active.id) return item;
        URL.revokeObjectURL(item.url);
        return { ...item, file: normalizedFile, url: normalizedUrl };
      }));
      if (outputUrl) URL.revokeObjectURL(outputUrl);
